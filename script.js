...
function addExpense(){
    ...
    if(expense < 1 || isNaN(expense) || expenseName.length <2){
        if(firstExpens){
...
function print(){
    ...
    printstr+= `<tr><th>${expenseNameForDetail[i]}</th><td>RS. ${expenseForDetail[i]}</td><td><button onclick=edit('${i}')>Edit</button></td><td><button onclick='deleteRecord(${i})'>Delete</button></td><tr>`;
    }
    return printstr;
}
function edit(a){
